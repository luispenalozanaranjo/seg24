...
if( isset($_POST['rut']) && $_POST['rut']!='' && isset($_POST['nombre']) && $_POST['nombre']!=''
	...
	&& isset($_POST['email']) && $_POST['email']!='' && isset($_POST['perfil']) && $_POST['perfil']!='' 
...
/*	print $valida.'    '.$cadena.'   '.$navegador['version'].' llll'.$_SESSION['glorut'].'navegador: '.$navegador['navegador'].' token:'.$token.' ffff:'.$valida.'   ddd:'.$valrut;*/
	if(/*!$valida!$valida ||*/ $valrut == 'NO'){
	 	session_destroy();
		...
		*/
		...
		$obj->Begin();
		//if($comuna!='')
		//$resultado = $obj->modificaUsuario($nombre,$paterno,$materno,$_POST['email'],$comuna,$perfil,$estado,$rut);
		//else
		//$resultado = $obj->modificaUsuarioV2($nombre,$paterno,$materno,$_POST['email'],$perfil,$estado,$rut);
		
		//if($obj->modificaUsuarioV2($nombre,$paterno,$materno,$_POST['email'],$perfil,$estado,$rut)==0)
		//$error++;
		$obj->modificaUsuarioV2($nombre,$paterno,$materno,$_POST['email'],$perfil,$estado,$rut);
		...
		if( $error == 0 ){
			...
			$auditoria_sql = "modificaUsuario(".$nombre.",".$paterno.",".$materno.",".$_POST['email'].",".$cmbcomunas.",".$perfil.",".$estado.",".$rut.")";
			$auditoria_meta = "Post";
